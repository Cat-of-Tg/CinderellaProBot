...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 7:
        pairs = pairs[modulo_page * 7:7 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("Pʀᴇᴠ", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("Hᴏᴍᴇ", callback_data="bot_start"),
             EqInlineKeyboardButton("Nᴇxᴛ", callback_data="{}_next({})".format(prefix, modulo_page)))]

    else:
        pairs += [[EqInlineKeyboardButton("Hᴏᴍᴇ", callback_data="bot_start")]]
